...
export function MessageInput({
  ...
  return (
    <form onSubmit={handleSubmit} className="relative p-4 bg-white border-t border-neutral-200/60" aria-busy={isLoading}>
      <div className="relative max-w-4xl mx-auto">
        ...
        >
          <Send aria-hidden="true" className={`h-4 w-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 ${isLoading || !input?.trim() ? 'text-neutral-300' : 'text-blue-600'}`} />
        </button>
      </div>
    </form>
  );
